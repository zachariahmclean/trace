...
```{r}
plot_traces(repeats_list[1], xlim = c(100, 150), show_peaks = FALSE)
```
...
```{r}
extract_trace_table(repeats_list[1]) |>
...
traces_df <- extract_trace_table(repeats_list)
...
traces_for_plotting_df <- extract_trace_table(repeats_list[samples_for_plotting])
...
alleles_df <- extract_alleles(repeats_list[samples_for_plotting]) |>
  filter(peak_allele == 1) # filter for the allele of interest
