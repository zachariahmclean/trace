...
fsa_list <- read_fsa(fsa_file_list)
...
for (i in seq_along(fsa_list)) {
  ...
  fsa_list[[i]]$plot_trace(
    xlim = c(80, 250)
  )
  
  # Add text annotation for allele_repeat
  text(
    x = fsa_list[[i]]$get_allele_peak()$allele_repeat, 
    y = fsa_list[[i]]$get_allele_peak()$allele_signal * 1.02, 
    labels = round(fsa_list[[i]]$get_allele_peak()$allele_repeat)
  )
}
...
This can be useful for downstream aggregation of experiments. For example, we could read in the fsa_list back into R in another script to calculate instability metrics.
